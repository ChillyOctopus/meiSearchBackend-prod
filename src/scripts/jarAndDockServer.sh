...
jar xf ../out/artifacts/meiSearchBackend_jar/meiSearchBackend.jar

# Copy the manifest file to the correct location
cp ../META-INF/manifest.txt ../out/production/meiSearchBackend/manifest.txt

# Create the application JAR including the extracted contents
jar -cvfm myapp.jar ../out/production/meiSearchBackend/manifest.txt -C ./ .

# Move the created JAR file to the appropriate directory after
mv myapp.jar ../src/serverCode
...
docker-compose down
echo
source ~/meiSearchBackend/src/scripts/rmAllDocker.sh # Run the remove all docker script inside this shell
echo
docker-compose up --build -d
rm src/serverCode/myapp.jar
